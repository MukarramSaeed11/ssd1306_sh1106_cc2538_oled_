...
{
    I2CMasterSlaveAddrSet(SSD1306_I2C_ADDRESS, false); 
    I2CMasterDataPut(0x40);
...
void SSD1306_DrawPixel(uint8_t x, uint8_t y, int color)             //int1 color = true//
{
...
void SSD1306_Dim(uint8_t dim)              //int1//
{
  uint8_t contrast;
  if (dim)
...
void SSD1306_Display(void)           //ftn for SH1106 oled Display
{
  ...
  for ( i = 0; i < height; i++)
    {
      ...
      for( j = 0; j < 8; j++)
      {
        for ( k = 0; k < width; k++, p++)
        {
          SSD1306_SendByte(ssd1306_buffer[p]);
        }
       }
      }
}
...
//void SSD1306_Display(void)    //ftn for SSD1306 oled Display
//{
...
//        SSD1306_SendByte(ssd1306_buffer[i]);
//        i++;
//      }
//      i--;
//    }
...
{
  for (uint16_t i = 640; i < (SSD1306_LCDWIDTH*SSD1306_LCDHEIGHT / 8); i++)
    ssd1306_buffer[i] = 0;
}

void SSD1306_DrawLine(int16_t x0, int16_t y0, int16_t x1, int16_t y1, int color)    //int1 color = true
{
  int steep;
  int8_t ystep;
  uint8_t dx, dy;
...
void SSD1306_DrawFastHLine(uint8_t x, uint8_t y, uint8_t w, int color)       //int color is true
{
   SSD1306_DrawLine(x, y, x + w - 1, y, color);
}

void SSD1306_DrawFastVLine(uint8_t x, uint8_t y, uint8_t h, int color)         //color true
{
  SSD1306_DrawLine(x, y, x, y + h - 1, color);
}

void SSD1306_FillRect(uint8_t x, uint8_t y, uint8_t w, uint8_t h, int color)    //color true
{
  for (int16_t i = x; i < x + w; i++)
    SSD1306_DrawFastVLine(i, y, h, color);
}

void SSD1306_FillScreen(int color) {                               //color true
...
void SSD1306_FillCircleHelper(int16_t x0, int16_t y0, int16_t r, uint8_t cornername, int16_t delta, int color) {//color true
  int16_t f     = 1 - r;
  int16_t ddF_x = 1;
  int16_t ddF_y = -2 * r;
  int16_t x     = 0;
...
void SSD1306_FillCircle(int16_t x0, int16_t y0, int16_t r, int color)      //color is true
{
...
void SSD1306_FillRoundRect(uint8_t x, uint8_t y, uint8_t w, uint8_t h, uint8_t r, int color)  //color true
{
  // smarter version
...
// Fill a triangle
void SSD1306_FillTriangle(int16_t x0, int16_t y0, int16_t x1, int16_t y1, int16_t x2, int16_t y2, int color)  //color true
{
  int16_t a, b, y, last;
...
void SSD1306_SetTextWrap(int w)           //int1 w
{
  wrap = w;
}
...
void SSD1306_PutCustomC(uint8_t *c)                     //rom uint8_t c??
{
  int _color;                 //int1 color
  uint8_t i, j, line;
...
void SSD1306_ROMBMP(uint8_t x, uint8_t y,uint8_t const *bitmap, uint8_t w, uint8_t h)
{
  ...
  {   
    ...
    {     
      if( SSD1306_BitTest(bitmap[j/8 + i*w], j % 8) == 1 )// % modülüs bölme islemi. Bölme islemi sonrasi kalani verir.
